...
public class QueueSymptom {
	...
	/**
	 * AddSymptom 
	 * Purge the List and the added Symptom will be sort.
	 * 
	 * @param s
	 * 		The symptom to add
	 */
	public void addSymptom(Symptom s){
		...
		this.purgeQueue();
		//To know if the symptom is add when it's compared to others in the list
		boolean add = false;
		...
		//If its priority is the lowest or egal to all the others symptoms, add at the end
		if(!add){
			this.queue.add(s);
		}
	}
	...
	/**
	 * Method to pull the symptom with the highest priority (the first in the List)
	 * @return First Symptom in a sorted List
	 * 		null if not symptom in the queue
	 */
	...
	public void purgeQueue(){
		for (Symptom s : this.queue)
			if(s.isExpired()) this.queue.remove(s);
	}
